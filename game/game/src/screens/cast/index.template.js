/* eslint-disable max-len */
export default `
<section id="cast" class="d-flex flex-column bd-highlight align-items-start">
<a href="#" title="закрыть" class="close-window"></a>
  <div class="btn-group-vertical btn-group-lg p-2 flex-fill bd-highlight">
    <button type="button" id="taskMath" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskTransl" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskLogic" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskGram" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskSpeakOut" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskFunnyLogic" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskMagicWord" class="cast__button d-flex flex-row justify-content-between align-items-center btn btn-success"></button>
    <button type="button" id="taskMystery" class="cast__button cast__button_footer d-flex flex-row justify-content-between align-items-center btn btn-warning"></button>
  </div>
  <footer class="d-flex flex-row justify-content-between cast__info-navigation">
    <div class="col"><span>Q</span> свернуть окно</div>
    <div class="col"><span>S</span> вкл/выкл звук</div>
    <div class="col"><span>&uarr; &darr;</span> вверх/вниз</div>
    <div class="col"><span>&larr; &rarr;</span> влево/вправо</div>
  </footer>
</section>
`;
